...
public class ConvertAnArrayIntoA2DArrayWithConditions {
    public static void main(String[] args) {
        System.out.println(findMatrix1(new int[]{1,3,4,1,2,3,1}));
        System.out.println(findMatrix1(new int[]{1,2,3,4}));
    }
    ...
    public static List<List<Integer>> findMatrix1(int[] nums) {
        ...
        while(true){
            boolean encounteredElement = false;
            List<Integer> subResult = new ArrayList<>();
            for(int i=1; i<=nums.length; i++){
                if(!encounteredElement && map[i] != 0)
